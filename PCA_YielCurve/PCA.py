...
#transform the dataset onto eigenvectors
...
df_pca_level = pd.DataFrame(df_pca_level, columns=[f'PCA_{x}' for x in range(df_pca_level.shape[1])])  # np.array to dataframe
df_pca_level.index = df.index
plt.figure(figsize=(15, 8))
plt.plot(df_pca_level['PCA_3'], label='first component')
plt.plot(df_pca_level['PCA_2'], label='second component')
plt.plot(df_pca_level['PCA_1'], label='third component')
plt.legend()
plt.show()
